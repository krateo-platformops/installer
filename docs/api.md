...
The DAG at a glance (26 components at 0.3.11): the `platform` tier
...
(`kagent-crds` → `kagent` → `fetch-mcp-server`, `krateo-autopilot`, the five
specialists + `clickhouse-mcp-server`, all pinned to the private
`oci://ghcr.io/krateo-agentiko/charts`).
