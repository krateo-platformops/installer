...
| `coreAgents` | `false` | The base agent layer: `kagent-crds` → `kagent` → `installer-agent` + `krateo-autopilot`. The autopilot component brings up `repo-mcp-server`, the grounding server every agent in the fleet reads through. |
