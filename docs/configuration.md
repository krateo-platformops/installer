...
| `structureGraph` | `false` | Opt-in **code-structure grounding**: `structure-graph-mcp-server` (a Graphify tree-sitter AST graph of the engine repos — `query_graph`/`shortest_path`/`get_neighbors`/`graph_stats`), so agents answer "what calls this", "shortest path between these symbols", "blast radius of changing this type" from a deterministic AST pass. Needs `coreAgents`. **One switch:** the two consumers (`krateo-autopilot`'s `mcpServers.structureGraph`, `core-provider-agent`'s `structureGraph`) are **derived** from this flag at render time and cannot disagree with it — previously they were separate values, and the two halves are broken apart (the feature alone deploys a server nothing names; a consumer naming an undeployed server makes kagent refuse to compile that Agent). Left **off by default** — but *not* for memory, which was measured and is a non-issue (256Mi request = 0.9% of a 27.7GiB node; the 1Gi is a limit, i.e. a ceiling the scheduler never reserves; the graph is baked at image-build time and the load path measures ~62MiB RSS locally). The real reasons: the pinned image **`0.1.1` does not boot** — ENTRYPOINT fails with `"HTTP transport needs the mcp extra"` because its `requirements.txt` omits `graphifyy[mcp]` on both published architectures, so enabling it today deploys a CrashLooping pod — and it needs `coreAgents`. In-cluster steady state under query load remains **unmeasured**: no Graphify-backed server has ever run on a Krateo cluster. |
