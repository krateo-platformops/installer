...
| `structureGraph` | `true` | **Code-structure grounding**, on by default: `structure-graph-mcp-server` (a Graphify tree-sitter AST graph of the engine repos — `query_graph`/`shortest_path`/`get_neighbors`/`graph_stats`), so agents answer "what calls this", "shortest path between these symbols", "blast radius of changing this type" from a deterministic AST pass instead of inference. Both consumers — `krateo-autopilot` (`mcpServers.structureGraph.enabled`) and `core-provider-agent` (`structureGraph.enabled`) — default `true` **with** it; the three move as one, since the feature alone deploys a server nothing names and a name with no server fails to compile. Needs `coreAgents`, so it only lands where the agent fleet does. **Cost:** the graph is held in memory (~1Gi limit on the server pod) — set all three `false` to opt out. |
